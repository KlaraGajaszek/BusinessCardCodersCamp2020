...
class Queen extends Piece {
    ...
    findLegalMoves(board: Board, actualField: Field): string[] {
        ...
        const possibleMoves = attacking.filter(
            (position) =>
                (board.fields[Number(position[0])][Number(position[2])].piece !== null &&
                    board.fields[Number(position[0])][Number(position[2])].piece!.side !== actualField.piece!.side) ||
                board.fields[Number(position[0])][Number(position[2])].piece === null,
        );
